...
(function () {
    ...
    document.body.addEventListener("click", function (e) {
        ...
        if (tar.className === "btn-post" || tar.parentNode.className === "btn-post") {
            if (content.innerHTML === "") {
                var flag = confirm("先随便说两句吧！");
                if (flag) {
                    content.focus();
                    return;
                }
            ...
            var str = "";
            //将.gif表情 转换为png 正则替换下图片地址中的后缀即可
            var val = content.innerHTML;
            val = val.replace(/src="http:\/\/cn.qzonestyle\.gtimg\.cn\/qzone\/em\/e(\d{3})\.gif"/g, function () {
                var newSrc = "src='http://cn.qzonestyle.gtimg.cn/qzone/em/e" + arguments[1] + ".png'";
                return newSrc;
            });
            var postTime=new Date();
            postTime=postTime.getMonth()+1+"月"+postTime.getDate()+"日";
            str += "<div class='f-aside'><div class='f-user-pto'><img src='./img/friends/friends-1.jpg' alt=''></div><div class='f-user-info'><a href='javascript:'>小人家</a><span>"+postTime+"</span><span><span class='scan" +
            " none'>浏览</span></span></div></div>";
            str += " <div class='f-wrap'><div class='f-info'>" + val + "</div>";
            str += "<div class='qz_summary'><div class='f-op-wrap'><p class='f-detail'><a href='javascript:;' class='item'><i class='ui-icon icon-comment commentsBtn'></i>评论<span class='none'></span></a><span class='item-line'></span><a href='javascript:;'class='item'>          <i class='ui-icon icon-forward'></i>转发<span class='none'></span></a><span class='item-line'></span><a href='javascript:;' class='item praise'><i class='ui-icon icon-praise'></i>赞<span class='none'></span></a><span class='item-line'></span><a href='javascript:;'class='item'><i class='ui-icon icon-feed-collect'></i>收藏<span class='none'></span></a> </p> <div class='mod-comments'> <div class='comments-list '><ul> </ul></div>";
            str += "<div class='addComment'><div class='addComment-box'>我也说一句</div><div class='addComment-inner none ' spellcheck='false' contenteditable='true'></div><div class='addComment-ft none'><a href='javascript:;' class='comment-btn-post'><span>发表</span> </a></div></div>";
            commentLi.innerHTML = str;
            str = "";
            content.innerHTML = "";
            commentLi.className = "f-single";
            oLis = utils.children(friendsList, "li");
            friendsList.insertBefore(commentLi, oLis[0]);
            bindClickEvent(commentLi);
            if (utils.hasClass(iniContent, "none")) {
                utils.removeClass(iniContent, "none");
                utils.addClass(content, "none");
            }
            if (!utils.hasClass(posterFt, "none")) {
...
for (var i = 0; i < oLis.length; i++) {
    (function (i) {
        if(!oLis.flag){
            bindClickEvent(oLis[i]);
            oLis.falg=true;
        }
    })(i)
}
var summary = null, summaryUl = null, summaryDetail = null, commentsBtn = null, numComment = null, modComment = null, commentsList = null, addComment = null, defaultSayBox = null, sayBox = null, sayFt = null;
function bindClickEvent(curEle) {
    ...
    //var praiseBtn = summaryDetail.getElementsByTagName("a", summaryDetail)[2];
    //var numComment = commentsBtn.getElementsByTagName("span")[0];
    //var commentsList = utils.getElementsByClass("mod-comments", summaryDetail)[0];
    //var defaultSayBox = utils.getElementsByClass("addComment-box", document)[0];
    //var sayBox = utils.next(defaultSayBox), sayFt = utils.next(sayBox);
    document.body.addEventListener("click", function (e) {
            e = e || window.event;
            var tar = e.target || e.srcElement;
            if (utils.hasClass(tar, "commentsBtn") || utils.hasClass(tar.parentNode, "commentsBtn") || utils.hasClass(tar, "addComment-box")) {
                if (utils.hasClass(tar, "commentsBtn")) {
                    summaryDetail = tar.parentNode;
                    commentsBtn = tar;
                    numComment = tar.getElementsByTagName("span")[0];
                    modComment = utils.next(summaryDetail);
                    commentsList = utils.getElementsByClass("comments-list", modComment)[0];
                    addComment = utils.getElementsByClass("addComment", modComment)[0];
                    defaultSayBox = utils.getElementsByClass("addComment-box", addComment)[0];
                    sayBox = utils.next(defaultSayBox);
                    sayFt = utils.next(sayBox);
                } else if (utils.hasClass(tar.parentNode, "commentsBtn")) {
                    summaryDetail = tar.parentNode.parentNode;
                    commentsBtn = tar.parentNode;
                    numComment = tar.parentNode.getElementsByTagName("span")[0];
                    modComment = utils.next(summaryDetail);
                    commentsList = utils.getElementsByClass("comments-list", modComment)[0];
                    addComment = utils.getElementsByClass("addComment", modComment)[0];
                    defaultSayBox = utils.getElementsByClass("addComment-box", addComment)[0];
                    ...
                    defaultSayBox = tar;
                    sayBox = utils.next(tar);
                    sayFt = utils.next(sayBox);
                    addComment = tar.parentNode;commentsList = utils.prev(addComment);
                    modComment =addComment.parentNode;
                    summaryDetail =utils.prev(modComment);
                    commentsBtn = summaryDetail.getElementsByTagName("a")[0];
                    numComment = commentsBtn.getElementsByTagName("span")[0];
                }
            ...
            if (tar.className === "comment-btn-post" || tar.parentNode.className === "comment-btn-post") {
                if(tar.className === "comment-btn-post"){
                    sayFt=tar.parentNode;
                }else{
                    sayFt=tar.parentNode.parentNode;
                }
                sayBox=utils.prev(sayFt);
                defaultSayBox=utils.prev(sayBox);
                addComment = sayBox.parentNode;commentsList = utils.prev(addComment);
                modComment =addComment.parentNode;
                summaryDetail =utils.prev(modComment);
                commentsBtn = summaryDetail.getElementsByTagName("a")[0];
                numComment = commentsBtn.getElementsByTagName("span")[0];
                summaryUl = commentsList.getElementsByTagName("ul")[0];
                //
                //if (sayBox.innerHTML == "") {
                //    var flag = confirm("请输入评论内容");
                //    if (flag) {
                //        return;
                //    }
                ...
                utils.addClass(sayFt, "none");
                //评论数+1
                if (utils.hasClass(numComment, "none")) {
                    utils.removeClass(numComment, "none");
                    numComment.innerHTML = "(1)";
                    return;
                }
                numComment.innerHTML = numComment.innerHTML.replace(/^\((\d+)\)$/, function () {
                    return "(" + (Number(arguments[1]) + 1) + ")";
                });
                return;
            }
            //点赞
            if (utils.hasClass(tar, "praise")||utils.hasClass(tar.parentNode, "praise")) {
                if(utils.hasClass(tar, "praise")){
                    var numPraise = tar.getElementsByTagName("span")[0];
                }else{
                    var numPraise = tar;
                }
                if (utils.hasClass(numPraise, "none")) {
                    utils.removeClass(numPraise, "none");
                    numPraise.innerHTML = "(1)";
                    return;
                }
                numPraise.innerHTML = numPraise.innerHTML.replace(/^\((\d+)\)$/, function () {
                    return "(" + (Number(arguments[1]) + 1) + ")";
                });
                return;
            }
            if (tar.className === "background-container" || tar.className === "col-main") {
                if (utils.hasClass(defaultSayBox, "none") && sayBox.innerHTML === "") {
                    utils.removeClass(defaultSayBox, "none");
                    utils.addClass(sayBox, "none");
                }
                if (!utils.hasClass(sayFt, "none")) {
                    utils.addClass(sayFt, "none");
                }
            }
        } ,false);
